...
```{r}
res_CAEAvsEA <- DESeq2::results(dds, contrast=c("Genotype","CAEA","EA"), alpha = 0.05)
summary(res_CAEAvsEA)

CAEAvsEA_list<-data.frame(gene = rownames(res_CAEAvsEA), res_CAEAvsEA, stringsAsFactors = F) %>% 
  na.omit() %>% 
  dplyr::select("log2FoldChange", "padj") %>% 
  tibble::rownames_to_column(var = "ensembl_gene_id_version") %>% 
  dplyr::mutate(padj = case_when(padj == 0 ~ .Machine$double.xmin,
...
CAEAvsEA_pos <- subset(CAEAvsEA_list, signed_padj > 0) %>%
  arrange(signed_padj, abs(log2FoldChange)) %>%
  mutate(rank = row_number()) %>%
  arrange(desc(rank))

CAEAvsEA_neg <- subset(CAEAvsEA_list, signed_padj < 0) %>%
  arrange(desc(signed_padj), abs(log2FoldChange)) %>%
  mutate(rank = -row_number()) %>%
  arrange(desc(rank))

CAEAvsEA_list <- bind_rows(CAEAvsEA_pos, CAEAvsEA_neg) %>% dplyr::arrange(desc(rank))

write.xlsx(CAEAvsEA_list, "TNFdataset/CAEAvsEA.xlsx")
```
...
ggplot(CAEA_EA_merged, aes(x=signed_padj.CAEA, y=signed_padj.EA))+
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  geom_text_repel(data=subset(CAEA_EA_merged, signed_padj.EA > 10 | 
                                signed_padj.EA < -12.5 |
                                signed_padj.CAEA > 50 |
                                signed_padj.CAEA < -25), 
                  aes(label=hgnc_symbol),
...
tnfUP_signature <- subset(TNFvsUT_list, padj <= 0.001 & log2FoldChange > log2(1.5))$ensembl_gene_id_version
...
ggplot(EAvsWT_list, aes(x=log2FoldChange, y=(minuslogp)))+
  geom_point(size=2,color="grey", alpha=0.3)+
  geom_point(data=subset(EAvsWT_list, padj < 0.001), size=2,color="darkred", alpha=0.4)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  xlab("log2FoldChange")+
  theme(axis.title = element_text(size = 12, face = "bold"), axis.text=element_text(size=10, face = "bold"))+
...
ggplot(CAEAvsWT_list, aes(x=log2FoldChange, y=(minuslogp)))+
  geom_point(size=2,color="grey", alpha=0.3)+
  geom_point(data=subset(CAEAvsWT_list, padj < 0.001), size=2,color="darkred", alpha=0.4)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  xlab("log2FoldChange")+
  theme(axis.title = element_text(size = 12, face = "bold"), axis.text=element_text(size=10, face = "bold"))+
...
pdf(file="TNFdataset/VolcanoPlots/Volcano_CATNFinteraction.pdf", width=6, height=6)
...
```{r}
discovery_genes <- c("ENSG00000104081.14")
