...
ggplot(pcaData, aes(PC1, PC2, color=Sample_Group)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line(), plot.title = element_text(size=20))+
    scale_color_manual(values=c("WT_PMATNF_UT" = "#B0B0B0",
                  "WT_PMA_2H" = "#696969",
...
PMA_CAvsWT_UP <- subset(PMA_CAvsWT_list, padj < 0.001 & log2FoldChange >= 0.5) %>% dplyr::arrange(desc(minuslogp))
PMA_CAvsWT_DN <- subset(PMA_CAvsWT_list, padj < 0.001 & log2FoldChange <= -0.5) %>% dplyr::arrange(desc(minuslogp))
PMAxCA_UP <- subset(CAPMAtreatment_list, padj < 0.001 & log2FoldChange >= 0.5) %>% dplyr::arrange(desc(minuslogp))
PMAxCA_DN <- subset(CAPMAtreatment_list, padj < 0.001 & log2FoldChange <= -0.5) %>% dplyr::arrange(desc(minuslogp))

# Create a new workbook
wb <- createWorkbook()

addWorksheet(wb, "PMA_CAvsWT_UP")
addWorksheet(wb, "PMA_CAvsWT_DN")
addWorksheet(wb, "PMAxCA_UP")
addWorksheet(wb, "PMAxCA_DN")

# Write the data to the sheet
writeData(wb, sheet = "PMA_CAvsWT_UP", PMA_CAvsWT_UP)
writeData(wb, sheet = "PMA_CAvsWT_DN", PMA_CAvsWT_DN)
writeData(wb, sheet = "PMAxCA_UP", PMAxCA_UP)
writeData(wb, sheet = "PMAxCA_DN", PMAxCA_DN)

  
# Optionally, you can set column widths if needed
setColWidths(wb, sheet = "PMA_CAvsWT_UP", cols = 1:ncol(PMA_CAvsWT_UP), widths = "auto")
setColWidths(wb, sheet = "PMA_CAvsWT_DN", cols = 1:ncol(PMA_CAvsWT_DN), widths = "auto")
setColWidths(wb, sheet = "PMAxCA_UP", cols = 1:ncol(PMAxCA_UP), widths = "auto")
setColWidths(wb, sheet = "PMAxCA_DN", cols = 1:ncol(PMAxCA_DN), widths = "auto")

# Save the workbook
saveWorkbook(wb, "Signatures_CAPMA.xlsx", overwrite = TRUE)
```
...
ggplot(CAPMAtreatment_list, aes(x=log2FoldChange, y=(minuslogp)))+
  ...
  geom_point(data=subset(CAPMAtreatment_list, abs(log2FoldChange) > 0.5 & padj < 0.001), size=3,color="darkred", alpha=0.6)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  ...
  ylab("-log10(p-adj)")+
  theme(axis.title = element_text(size = 12, face = "bold"), axis.text=element_text(size=10, face = "bold"))+
  geom_text_repel(data=subset(CAPMAtreatment_list, log2FoldChange < -0.5 & minuslogp > 30), aes(label=unique_gene_id),
                  size=4,
                  color="black",
                  nudge_y = 5,
                  force=20,
...
pdf(file="PMAdataset/VolcanoPlots/Volcano_CAPMATNFtreatment_Interaction.pdf", width=6, height=6)
ggplot(CAPMATNFtreatment_list, aes(x=log2FoldChange, y=(minuslogp)))+
  geom_point(size=2.25,color="grey", alpha=0.3)+
  geom_point(data=subset(CAPMATNFtreatment_list, abs(log2FoldChange) > 0.5 & padj < 0.001), size=3,color="darkred", alpha=0.6)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  xlab("log2FoldChange")+
  theme(axis.title = element_text(size = 12, face = "bold"), axis.text=element_text(size=10, face = "bold"))+
...
ggplot(PMA_CAvsWT_list, aes(x=log2FoldChange, y=(minuslogp)))+
  geom_point(size=2.25,color="grey", alpha=0.3)+
  geom_point(data=subset(PMA_CAvsWT_list, abs(log2FoldChange) > 0.5 & padj < 0.001), size=3,color="darkred", alpha=0.6)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(),plot.background = element_blank(), axis.line = element_line()) +
  xlab("log2FoldChange")+
  theme(axis.title = element_text(size = 12, face = "bold"), axis.text=element_text(size=10, face = "bold"))+
