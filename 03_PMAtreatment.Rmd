...
PMATNF_CAvsWT_gsea <- subset(PMATNF_CAvsWT_list, padj <= 0.05)
PMATNF_CAvsWT_ranked<-PMATNF_CAvsWT_gsea$log2FoldChange
names(PMATNF_CAvsWT_ranked)<-str_remove(PMATNF_CAvsWT_gsea$ensembl_gene_id_version, "\\..*$")
PMATNF_CAvsWT_ranked <- sort(PMATNF_CAvsWT_ranked, decreasing=TRUE)

reac_enrich_PMATNF_CAvsWT_GSEAobject <- GSEA(PMATNF_CAvsWT_ranked, TERM2GENE = reactome_genesets, eps = 0, pvalueCutoff = 0.1, nPermSimple = 1e6)
